...
public class Circulo1 {
	...
	BMP_Gen gen = new BMP_Gen();
		...
		int [][] dibujo = new int [410] [410];
		int [] pixels = new int[dibujo.length * dibujo[0].length];

		while(i < 2000){
			if(((dibujo.length) < 5 + (int)(200.0 * (1.0 + Math.cos((double)(i) * Math.PI / 1000.0)))) || ((dibujo[0].length) < 5 + (int)(200.0 * (1.0 + Math.sin((double)(i) * Math.PI / 1000.0))))){
			System.out.println("Los argumentos de la sentencia plot no están dentro del rango permitido.");
			System.exit(-1);
			}
			dibujo[5 + (int)(200.0 * (1.0 + Math.cos((double)(i) * Math.PI / 1000.0)))][5 + (int)(200.0 * (1.0 + Math.sin((double)(i) * Math.PI / 1000.0)))] = 5;
			i = i + 1;
